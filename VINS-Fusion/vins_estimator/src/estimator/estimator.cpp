...
void Estimator::inputIMU(double t, const Vector3d &linearAcceleration, const Vector3d &angularVelocity)
{
    ...
    if (solver_flag == NON_LINEAR)
    {
        ...
        pubLatestOdometry(latest_P, latest_Q, latest_V, t); // todo
        mPropagate.unlock();
    }
}
                                ...
                                vector<pair<double, Eigen::Vector3d>> &gyrVector)
{
    ...
    {//如果t1时间大于imu最新时间，(imu米有了)
        printf("wait for imu\n");
        return false;
    }
    return true;
}
//如果imu buffer不空 且 图像时间戳+td<=imu最新时间戳
...
void Estimator::processMeasurements()
{
    while (1)
    {
        ...
        if(!featureBuf.empty())
        {
            ...
            mBuf.lock();
            if(USE_IMU)//获取(prevTime,curTime]之间的imu数据
